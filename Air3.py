...
def get_urls():
    sas_url = f"https://privateproperty.blob.core.windows.net/airbnb/Listingids3.csv?sv=2023-01-03&st=2025-01-13T07%3A50%3A48Z&se=2027-01-14T07%3A50%3A00Z&sr=c&sp=racwdxltf&sig=EUqQibbbiaeiTEE7fX%2FK8SS5j6%2Fao%2F%2BtmrS5z2Tzbm4%3D"
...
def ingest_data(all_data):
    ...
    if all_data:
        ...
        sas_url = f"https://privateproperty.blob.core.windows.net/airbnb/AirbnbListings3{start_time}.csv?sv=2023-01-03&st=2025-01-13T07%3A50%3A48Z&se=2027-01-14T07%3A50%3A00Z&sr=c&sp=racwdxltf&sig=EUqQibbbiaeiTEE7fX%2FK8SS5j6%2Fao%2F%2BtmrS5z2Tzbm4%3D"
        # Create BlobClient
        client = BlobClient.from_blob_url(sas_url)
        # Upload blob
...
def ingest_removed_id(unavailable_ids):
    ...
    if unavailable_ids:
        ...
        car_data_csv = data_frame.to_csv(encoding="utf-8", index=False).encode("utf-8")
        sas_url = f"https://privateproperty.blob.core.windows.net/airbnb/Air_removed3{start_time}.csv?sv=2023-01-03&st=2025-01-13T07%3A50%3A48Z&se=2027-01-14T07%3A50%3A00Z&sr=c&sp=racwdxltf&sig=EUqQibbbiaeiTEE7fX%2FK8SS5j6%2Fao%2F%2BtmrS5z2Tzbm4%3D"
        # Create BlobClient
        client = BlobClient.from_blob_url(sas_url)
        # Upload blob
        client.upload_blob(car_data_csv, overwrite=True)
